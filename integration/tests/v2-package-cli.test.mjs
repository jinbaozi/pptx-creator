...
describe("V2 package and verify wrappers", () => {
  ...
  it("fails before packaging when an independent Skill boundary is incomplete", async () => {
    ...
    try {
      fs.mkdirSync(path.join(fixtureRoot, "integration", "protocol"), { recursive: true });
      fs.mkdirSync(path.join(fixtureRoot, "integration", "scripts"), { recursive: true });
      fs.copyFileSync(
        path.join(root, "integration", "protocol", "presentation-package.schema.json"),
        path.join(fixtureRoot, "integration", "protocol", "presentation-package.schema.json")
      );
      fs.copyFileSync(
        path.join(root, "integration", "scripts", "validate-presentation-package.mjs"),
        path.join(fixtureRoot, "integration", "scripts", "validate-presentation-package.mjs")
      );
