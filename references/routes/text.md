...
npm run pptx -- text <deck.html|plan-directory|deck.plan.json> <output-dir> [--design-system <path-or-name>]
...
On the default route, the Host-authored CSS is the visual system. An optional
built-in `--design-system` value supplies portable PowerPoint theme metadata;
it must not replace or restyle the accepted HTML. Existing `DESIGN.md` path
selection remains available in `--native` compatibility mode.
...
`deck.manifest.json`, `replica-evidence.json`, editability and QA reports,
`final.pptx`, and `output-manifest.json`. The `--native` compatibility route
