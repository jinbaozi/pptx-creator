...
The public creative route accepts `--design-system <path-or-name>`. An explicit user-requested style, brand, template, or visual reference wins; an explicitly supplied local `DESIGN.md` is also a style lock. When the user supplies no style, the Host selects a direction from the audience, content, delivery environment, language, readability, and emotional tone, then passes that decision into authoring or `--design-system`. Topic keywords do not automatically select `dark-tech` or any other built-in. Existing local files or directories resolve before built-in names. If neither the user nor the Host makes a selection, the deterministic resolver still checks repository-root `DESIGN.md`, input-adjacent `DESIGN.md`, then built-in `business-neutral`; this is a safety fallback, not a creative preference. The selected file is parsed exactly once before compilation; its name is derived from frontmatter, resolved tokens are materialized into native element styling, and the packaged manifest keeps the portable source `design-system/DESIGN.md`. Selection provenance records the original request and resolved local file under `metadata.designIntent.designSystemSelection`.
