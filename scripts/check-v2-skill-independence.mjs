...
async function inspectSkill(skillRoot, canonicalSchemaBytes, canonicalValidatorBytes, repositoryRoot) {
  ...
  for (const candidate of protocolPaths) {
    try {
      ...
      if (digest(bytes) !== digest(canonicalSchemaBytes)) {
  ...
  try {
    ...
    if (digest(validatorBytes) !== digest(canonicalValidatorBytes)) {
...
export async function checkV2SkillIndependence(repositoryRoot) {
  const canonicalSchemaBytes = await readFile(join(repositoryRoot, "schemas", "presentation-package.schema.json"));
  const canonicalValidatorBytes = await readFile(join(repositoryRoot, "scripts", "validate-presentation-package.mjs"));
  const reports = [];
