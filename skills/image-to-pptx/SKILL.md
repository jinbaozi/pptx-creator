...
The command copies source files, performs OCR and geometry analysis, creates native-first PPTX objects, renders the candidate, compares every page with its reference, applies at most three bounded text calibrations, and publishes `final.pptx` only after the blocking quality gate passes. It also emits an optional interoperable HTML reconstruction package unless `--no-html-package` is used.
...
Treat the task as complete only when `qa-report.json` says `passed`, `final.pptx` exists, the preview pages were rendered from that exact file, all declared local raster regions are bounded, and the protocol validator passes when the optional HTML package is emitted.
